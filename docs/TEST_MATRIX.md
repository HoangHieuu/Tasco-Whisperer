...
| US-019 | Generalized Vietnamese query intelligence reduces fixture dependence. | yes | yes | no | no | implemented | `src/lib/vietnamese.test.ts` covers syllable segmentation and Telex/VNI cleanup; engine tests prove deterministic compact handling without agent calls. |
| US-020 | Semantic retrieval source adds dataset-grounded candidates beyond exact rules. | yes | yes | no | no | implemented | `src/lib/semantic.test.ts` covers similarity scoring and dataset-grounded vector candidate generation; semantic and embedding sources are ranked through existing score factors. |
...
| US-025 | Ranking weight tuning is explicit, measurable, and reproducible. | yes | yes | no | no | implemented | `src/lib/engine.test.ts` covers `rankingWeights`; `npm run rank:tune` writes `reports/ranking-tuning/latest.json` and `.md` with preset comparisons. |
