...
| US-008 | Ranking engine produces transparent scores and source/reason metadata. | yes | yes | no | no | implemented | `npm run eval`: top1 88.3%, top3 100%, top5 100%, MRR 0.922, p95 17 ms in the latest full check; UI score-factor panel shows ranking factors. |
| US-009 | Local/simulated personalization boosts relevant suggestions with fallback. | yes | yes | yes | no | implemented | `npm run test` covers baseline fallback, known profile boosts, unknown profile fallback, `agentic=false`, and API metadata with `personalizationReason`; browser smoke verifies the Coffee loyalist profile reason in the UI. |
| US-010 | Public evaluation harness reports accuracy, recall, MRR, difficulty/type metrics, and latency. | yes | yes | no | no | implemented | `npm run eval`: 60 cases, top1 88.3%, top3 100%, top5 100%, intent 51.7%, MRR 0.922, p95 latency 17 ms in the latest full check. |
...
| US-014 | Agentic query understanding adds validated hard-case analysis with deterministic fallback. | yes | yes | yes | no | implemented | `npm run check` passes with 40 tests; tests cover structured parser validation, deterministic compact-prefix fallback, `caphe -> cà phê`, negative rewrite fixtures, API diagnostics, and alias-memory reuse; browser smoke verifies `cap` and `cayx` in the UI with no warnings/errors. |
| US-015 | Agentic evaluation loop proposes accepted, measurable tuning changes. | yes | yes | no | no | implemented | `npm test -- --run src/lib/agenticTuning.test.ts` covers weak-case analysis and advisory proposals; `npm run tune:agentic` exports JSON/Markdown reports with 30 weak cases, 4 proposals, and acceptance guardrails. |
