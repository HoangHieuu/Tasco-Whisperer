...
| US-008 | Ranking engine produces transparent scores and source/reason metadata. | yes | yes | no | no | implemented | `npm run check`: top1 96.7%, top3 100%, top5 100%, MRR 0.983; UI score-factor panel shows ranking factors; `npm run rank:tune` compares explicit weight presets. |
...
| US-010 | Public evaluation harness reports accuracy, recall, MRR, difficulty/type metrics, and latency. | yes | yes | no | no | implemented | `npm run check`: 60 cases, top1 96.7%, top3 100%, top5 100%, intent 98.3%, MRR 0.983, p95 latency 40 ms. |
...
| US-030 | Real-AI generalization runtime adds model-backed semantic context, runtime rewrite providers, and credibility fixes with deterministic fallback. | yes | yes | no | no | implemented | `npm run embeddings:build` writes a 316-document MiniLM artifact; `npm run eval:minilm` measures the async server path at top1 96.7%, top3 100%, intent 98.3%, MRR 0.983, p95 28 ms, 0 degraded cases; sync `npm run eval` remains top1 96.7%, top3 100%, intent 98.3%, MRR 0.983; `npm run eval:robust` reports 192 cases, top3/top5 100%, compact 53/53; templates are pruned to 83 rendered entries with eval guards; accepted hosted/local rewrites write back to alias memory during serving; `npm run test`, `npm run api:smoke`, `npm run build`, and `npm run check` pass. |
